...
$baso = mysqli_real_escape_string($conn, $_POST["baso"]);
...
WHERE A.no_so = '$baso' GROUP BY E.pluid_so";
    ...
    if(mysqli_num_rows($query_d) > 0 ) {
        ...
        if(mysqli_num_rows($query_d) > 0 ) {
            ...
            while($data = mysqli_fetch_assoc($query_d)){
                ...
                $pdf->SetWidths(array(10, 74, 16, 18, 18, 18, 18, 18));
                $pdf->Row(array($no++, $desc, $satuan, $stock, $asset, $fisik, $selisih_stock, $selisih_asset));
            ...
            $pdf->Cell(100 ,8,'Total',1,0,'');
            $pdf->Cell(18 ,8,$total_stock ,1,0,'');
            $pdf->Cell(18 ,8,$total_asset,1,0,'');
            $pdf->Cell(18 ,8,$total_fisik,1,0,'');
            $pdf->Cell(18 ,8,$total_selisih_stock,1,0,'');
            $pdf->Cell(18 ,8,$total_selisih_asset,1,1,'');
...
$pdf->Output("BASO-NO-".$header['no_so'].".pdf","I");
