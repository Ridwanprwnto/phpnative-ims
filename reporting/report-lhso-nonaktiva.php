...
if (isset($_SESSION['PRINTLHSONA'])){
    $_POST = $_SESSION['PRINTLHSONA'];
    unset($_SESSION['PRINTLHSONA']);
}
else {
...
class PDF extends PDF_MC_Table
{
    ...
    function Header()
    {
        ...
        $this->Cell(190 ,10,'LAPORAN HASIL STOCK OPNAME NON AKTIVA',0,0, 'C');
        ...
        $this->Cell(25 ,8,'Selisih',1,0,'C');
        $this->Cell(55 ,8,'Keterangan',1,1,'C');
...
$pdf->SetTitle('Report LHSO Barang Non AKtiva');
$pdf->SetSubject('Report LHSO Non Aktiva');
$pdf->SetKeywords('LHSO');
$pdf->SetCreator('IMS');
...
if(mysqli_num_rows($query_d) > 0 ) {
    ...
    while($data = mysqli_fetch_assoc($query_d)){
        ...
        $ket = $data["keterangan_so"];
        $selisih = $fisik - $saldo;

        $pdf->SetWidths(array(10, 50, 25, 25, 25, 55));
        $pdf->Row(array($no++, $desc, $saldo, $fisik, $selisih, $ket));

    }

}

$pdf->Output("LHSO-".$header['no_so']."-".substr($header['tgl_so'], 0, 10).".pdf","I");
