...
$title = "REKAPITULASI DATA BAK PERIODE";
...
$spreadsheet = new Spreadsheet();
    ...
    $sheet->setCellValue('I1', 'KERUSAKAN');
    ...
    if (isset($office) && isset($dept) && isset($awal) && isset($akhir) && isset($user) && isset($barang)) {
        ...
        if(mysqli_num_rows($query_xcl) > 0 ) {
            ...
            while($data_xcl = mysqli_fetch_assoc($query_xcl)){
                ...
                $slash_date  =  str_replace('-"', '/', $data_xcl['tgl_bkse']);
                $newDate  =  date( "d/m/Y", strtotime($slash_date));

                $docno = $data_xcl['nomor_bkse'];
                $desc = $data_xcl["pluid_bkse"]." - ".$data_xcl["NamaBarang"]." ".$data_xcl["NamaJenis"];
                $sn = $data_xcl['sn_bkse'];
                $at = $data_xcl['at_bkse'];
                $pic = $data_xcl["user_bkse"]." - ".strtoupper($data_xcl["pic"]);
                $lokasi = $data_xcl['penempatan_bkse'];
                $rusak = $data_xcl['kerusakan_bkse'];

                $sheet->setCellValue('A'.$row_xcl, $no++);
                $sheet->setCellValue('B'.$row_xcl, $docno);
                $sheet->setCellValue('C'.$row_xcl, $newDate);
                $sheet->setCellValue('D'.$row_xcl, $desc);
                $sheet->setCellValue('E'.$row_xcl, $sn);
                $sheet->setCellValue('F'.$row_xcl, $at);
                $sheet->setCellValue('G'.$row_xcl, $lokasi);
                $sheet->setCellValue('H'.$row_xcl, $pic);
                $sheet->setCellValue('I'.$row_xcl, $rusak);
                $row_xcl++;
            }
        }
        else {
    ...
    header('Content-Disposition: attachment; filename='.$title.' '.$awal.' '.$akhir.'.xlsx');
...
elseif (isset($_POST["printpdf"])) {
    ...
    $pdf->Output($title." ".$awal." ".$akhir.".pdf","I");
