...
$idsj = mysqli_real_escape_string($conn, $_POST["idsj-perbaikan"]);
$idbr = mysqli_real_escape_string($conn, $_POST["idbr-perbaikan"]);
$idsn = mysqli_real_escape_string($conn, $_POST["idsn-perbaikan"]);
...
WHERE A.head_no_sj = '$idsj' AND A.pluid_sj = '$idbr' AND A.sn_sj = '$idsn'";
...
$pdf->SetTitle('Reprint Bukti HPPB');

if (isset($idsj) && !empty($idbr) && !empty($idsn)) {
    ...
    if($header) {
        ...
        $pdf->Cell(120 ,28,'',1,0,'');
        $pdf->Cell(70 ,28,'',1,1,'');
        $pdf->Cell(120 ,8,'SPV ADMIN',1,0,'C');
