...
$title = $office." - LAPORAN SUMMARY PERALATAN INVENTARIS";
...
elseif (isset($_POST["printexcell"])) {
    ...
    $sheet->setCellValue('D2', 'CADANGAN');
    $sheet->setCellValue('E2', 'RUSAK');
    $sheet->setCellValue('F2', 'PERBAIKAN');
    $sheet->setCellValue('G2', 'P3AT');
    $sheet->setCellValue('H2', 'HILANG');
    ...
    if (isset($office) && isset($dept) && isset($barang)) {
        ...
        if(mysqli_num_rows($query_xcl) > 0 ) {
            ...
            while($data_xcl = mysqli_fetch_assoc($query_xcl)){
                ...
                $desc = $data_xcl["pluid"].' - '.$data_xcl['NamaBarang']." ".$data_xcl['NamaJenis'];
                $satu = $data_baik["total_baik"];
                $dua = $data_cadangan["total_cadangan"];
                $tiga = $data_rusak["total_rusak"];
                $empat = $data_service["total_service"];
                ...
                $sheet->setCellValue('B'.$row_xcl, $desc);
                $sheet->setCellValue('C'.$row_xcl, $satu);
                $sheet->setCellValue('D'.$row_xcl, $dua);
                $sheet->setCellValue('E'.$row_xcl, $tiga);
                $sheet->setCellValue('F'.$row_xcl, $empat);
                $sheet->setCellValue('G'.$row_xcl, $lima);
                $sheet->setCellValue('H'.$row_xcl, $tujuh);
                $sheet->setCellValue('I'.$row_xcl, $total);
                $row_xcl++;
