...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(isset($_POST["IDSO"]) && !empty($_POST["IDSO"]) && isset($_POST["LOKASISO"]) && !empty($_POST["LOKASISO"]) && isset($_POST["STATUSSO"]) && !empty($_POST["STATUSSO"]) && isset($_POST["USERSO"]) && !empty($_POST["USERSO"])){
        ...
        $user = mysqli_real_escape_string($conn, strtoupper($_POST['USERSO']));
        ...
        // Update to database
        mysqli_query($conn, "UPDATE asset_stock_opname SET kondisi_so_asset = '$kondisi', lokasi_so_asset = '$lokasi', user_so_asset = '$user', edtdate_so_asset = '$date', edited_so_asset = 'Y' WHERE id_so_asset = '$id'");
    ...
    if(isset($_POST["DETAILDATASANKSIPLG"]) && !empty($_POST["DETAILDATASANKSIPLG"])){
        ...
        $no = 1;
        $sql = "SELECT A.username_plg_cctv, B.*, C.id_office, D.id_department, E.id_divisi, E.divisi_name, J.name_fup_plg FROM user_pelanggaran_cctv AS A
        ...
        WHERE LEFT(A.username_plg_cctv, 10) = '$nik' AND B.office_plg_cctv = '$office' AND B.dept_plg_cctv = '$dept' AND LEFT(B.tgl_plg_cctv, 7) = '$month' AND B.status_plg_cctv = 'N' OR LEFT(A.username_plg_cctv, 10) = '$nik' AND B.office_plg_cctv = '$office' AND B.dept_plg_cctv = '$dept' AND LEFT(B.tgl_plg_cctv, 7) = '$month' AND B.status_plg_cctv = 'Y' ORDER BY A.head_no_plg_cctv ASC";
        ...
        <div class="form-row" >
            <div class="col-md-12 mb-2">
                <label>USER PELANGGAR</label>
                <input type="text" class="form-control" value="<?= substr($id, 15); ?>" disabled>
            </div>
            <div class="col-md-12 mb-2">
                ...
                <?php
                    while ($data = mysqli_fetch_assoc($query)) { 
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="badge badge-<?= $data['status_plg_cctv'] == 'N' ? 'warning' : 'success'; ?> label-square">
                                    <i class="ft-info font-medium-2"></i>
                                    <span><?= $data['status_plg_cctv'] == 'N' ? 'FUP' : 'APPROVE'; ?></span>
                                </div>
                            </td>
                        </tr>
                <?php 
    ...
    if(isset($_POST["DETAILDATASANKSIPLGALL"]) && !empty($_POST["DETAILDATASANKSIPLGALL"])){

        $id = $_POST['DETAILDATASANKSIPLGALL'];
        $office = substr($id, 0, 4);
        $dept = substr($id, 4, 4);
        $nik = substr($id, 8, 10);
        ...
        INNER JOIN divisi AS E ON B.div_plg_cctv = E.id_divisi
        INNER JOIN fup_pelanggaran AS J ON B.fup_plg_cctv = J.id_fup_plg
        WHERE LEFT(A.username_plg_cctv, 10) = '$nik' AND B.office_plg_cctv = '$office' AND B.dept_plg_cctv = '$dept' AND B.status_plg_cctv = 'N' OR LEFT(A.username_plg_cctv, 10) = '$nik' AND B.office_plg_cctv = '$office' AND B.dept_plg_cctv = '$dept' AND B.status_plg_cctv = 'Y' ORDER BY A.head_no_plg_cctv ASC";
    ...
    if(isset($_POST["ACTIONDETAILMONITORASSESSMENT"]) && !empty($_POST["ACTIONDETAILMONITORASSESSMENT"])){
        ...
        $sql = "SELECT A.head_id_sts_assest AS ID_ASSESST, A.docno_data_assest AS DOCNO_ASSESST, A.th_data_assest AS THN_ASSESST, A.date_data_assest AS DATE_ASSESST, CONCAT(A.leader_data_assest, ' - ', B.username) AS LEADER_ASSESST, A.poin_data_assest AS POIN_ASSESST, A.mutu_data_assest AS GRADE_ASSESST, A.status_data_assest AS STATUS_ASSESST, C.flag_sts_assest AS FLAG_ASSESST FROM data_assessment AS A
        INNER JOIN users AS B ON A.leader_data_assest = B.nik
        INNER JOIN statusassessment AS C ON A.head_id_sts_assest = C.id_sts_assest
        ...
        if ($query) {
            while($result = mysqli_fetch_assoc($query)) {
                $result = array(
                    ...
                    "FLAG_ASSESST"=>$result["FLAG_ASSESST"],
                );
                $row[] = $result;
            }
            die(json_encode(array(
    ...
    if(isset($_POST["ACTIONMONITORINGASSESSMENTLAPOR"]) && !empty($_POST["ACTIONMONITORINGASSESSMENTLAPOR"])){
        ...
        $sql = "SELECT A.head_id_sts_assest, A.docno_data_assest, A.leader_data_assest, A.th_data_assest, A.office_data_assest, A.div_data_assest, B.code_sts_assest, C.full_name FROM data_assessment AS A
        INNER JOIN statusassessment AS B ON A.head_id_sts_assest = B.id_sts_assest
        INNER JOIN users AS C ON A.leader_data_assest = C.nik
