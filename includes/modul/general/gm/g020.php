...
elseif(isset($_POST["deletedata"])){
    if(DeleteKKSO($_POST) > 0 ){
        $alert = array("Success!", "Data SO berhasil di batalkan", "success", "$redirect");
    }
    else {
        echo mysqli_error($conn);
    }
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Stock Opname Barang</h4>
                                    ...
                                    <form action="" method="post" role="form">
                                        ...
                                        <div class="modal-body">
                                            <div class="form-row">
                                                ...
                                                <div class="col-md-12 mb-2">
                                                    <label>Barang : </label>
                                                    <select class="select2 form-control" data-placeholder="Please Select" multiple="multiple" style="width: 100%" type="text" name="barangso[]">
                                                        <?php
                                                            $sql_barang = "SELECT A.pluid, A.no_lambung, A.sn_barang, B.IDJenis, B.NamaJenis, C.IDBarang, C.NamaBarang FROM barang_assets AS A
                                                            INNER JOIN masterjenis AS B ON RIGHT(A.pluid, 4) = B.IDJenis
                                                            INNER JOIN mastercategory AS C ON LEFT(A.pluid, 6) = C.IDBarang
                                                            WHERE A.ba_id_office = '$idoffice' AND A.ba_id_department = '$iddept' GROUP BY A.pluid ORDER BY C.NamaBarang ASC";
                                                            $query_barang = mysqli_query($conn, $sql_barang);
                                                            while($data_barang = mysqli_fetch_array($query_barang)) {
                                                        ?>
                                                            <option value="<?= $data_barang['pluid'];?>"><?= $data_barang['pluid']." - ".$data_barang['NamaBarang']." ".$data_barang['NamaJenis']; ?></option>
                                                        <?php
                                                            } 
                                                        ?>
                                                    </select>
                        ...
                        <div class="table-responsive">
                            <table
                                ...
                                <?php
                                    ...
                                    LEFT JOIN detail_stock_opname AS B ON A.no_so = B.no_so_head
                                    ...
                                    WHERE A.office_so = '$idoffice' AND A.dept_so = '$iddept' AND A.jenis_so = 1 AND A.status_so = 'N' GROUP BY A.no_so";
                                    ...
                                    <tr>
                                        ...
                                        <span class="dropdown">
                                            ...
                                            <span aria-labelledby="action<?= $data['no_so']; ?>" class="dropdown-menu mt-1 dropdown-menu-right">
                                                <a href="#" class="dropdown-item detail_draftso" title="Detail Data SO Nomor <?= $data["no_so"]; ?>" data-toggle="tooltip" data-placement="bottom" name="detail_draftso" id="<?= $data['no_so'].$idoffice.$iddept; ?>"><i class="ft-eye"></i>Detail SO</a>
                                                <a href="index.php?page=<?= $encpid;?>&ext=<?= encrypt($arrextmenu[0]);?>&id=<?= encrypt($data["no_so"]); ?>" title="Proses SO Nomor <?= $data["no_so"]; ?>" data-toggle="tooltip" data-placement="bottom" class="dropdown-item"><i class="ft-edit"></i>Proses SO</a>
                                                <a href="reporting/report-kkso.php?kkso=<?= encrypt($data['no_so']);?>" title="Print KKSO Nomor <?= $data["no_so"]; ?>" data-toggle="tooltip" data-placement="bottom" class="dropdown-item" onclick="document.location.href='<?= $redirect;?>'" target="_blank" ><i class="ft-printer"></i>Print KKSO</a>
                                        ...
                                        <form action="" method="post">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <input class="form-control" type="hidden" id="del-nodataso" name="del-noso" value="<?= $data["no_so"];?>" readonly>
