    ...
    <section id="minimal-statistics">
        ...
        <div class="row" id="row-color">
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title">Daftar Approval Sanksi Atas Users Tercatat Pelanggaran CCTV</h4>
                <a class="heading-elements-toggle"><i class="la la-ellipsis-v font-medium-3"></i></a>
                <div class="heading-elements">
                  <ul class="list-inline mb-0">
                    <li><a data-action="collapse"><i class="ft-minus"></i></a></li>
                ...
                <div class="card-body">
                    ...
                    <table class="table">
                        ...
                        <?php
                            $nocctv = 1;
                            $nol = [0, 0, 0, 0, 0];
                            $sql_userp = "SELECT A.no_plg_cctv, SUM(IF(A.fup_plg_cctv = '1', 1, 0)) AS jumlah_tl, SUM(IF(A.fup_plg_cctv = '3', 1, 0)) AS jumlah_st, SUM(IF(A.fup_plg_cctv = '4', 1, 0)) AS jumlah_sp, SUM(IF(A.fup_plg_cctv = '5', 1, 0)) AS jumlah_tt, A.office_plg_cctv, A.dept_plg_cctv, B.username_plg_cctv, COUNT(LEFT(B.username_plg_cctv, 10)) AS jumlah_plg_cctv, C.nik, D.divisi_name FROM pelanggaran_cctv AS A 
                            INNER JOIN user_pelanggaran_cctv AS B ON A.no_plg_cctv = B.head_no_plg_cctv 
                            LEFT JOIN users AS C ON LEFT(B.username_plg_cctv, 10) = C.nik 
                            LEFT JOIN divisi AS D ON C.id_divisi = D.id_divisi 
                            WHERE A.office_plg_cctv = '$office_id' AND A.dept_plg_cctv = '$dept_id' AND A.status_plg_cctv = 'Y' GROUP BY LEFT(B.username_plg_cctv, 10) ORDER BY COUNT(LEFT(B.username_plg_cctv, 10)) DESC";
                            ...
                            if(mysqli_num_rows($query_userp) > 0 ) {
                                while($data_userp = mysqli_fetch_assoc($query_userp)) { 
                                    ...
                                    $jumlah_tl = $data_userp['jumlah_tl'];
                                    $jumlah_st = $data_userp['jumlah_st'];
                                    $jumlah_sp = $data_userp['jumlah_sp'];
                                    $jumlah_tt = $data_userp['jumlah_tt'];
                                    $jumlah_plg_cctv = $data_userp['jumlah_plg_cctv'];
                                    ...
                                    if ($jumlah_plg_cctv >= 2) {
                                    ?>
                                        <tr <?= $color_stck; ?>>
                                            <td><?= $nocctv++; ?></td>
                                            <td><?= $bagian_plg_cctv; ?></td>
                                            <td><a title="Show Detail Data Pelanggaran User : <?= $user_plg_cctv; ?>" href="javascript:void(0);" data-toggle="tooltip" data-placement="bottom" class="text-<?= $color_txt; ?> detail_spc" name="detail_spc" id="<?= $office_plg_cctv.$dept_plg_cctv.$user_plg_cctv; ?>"><?= $user_plg_cctv; ?></a></td>
                                            <td><?= $jumlah_tl; ?></td>
                                            <td><?= $jumlah_st; ?></td>
                                            <td><?= $jumlah_sp; ?></td>
                                            <td><?= $jumlah_tt; ?></td>
