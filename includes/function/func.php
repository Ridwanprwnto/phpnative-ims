...
function ProsesBarangStock($data) {
    ...
    if ($qty_terima != $qty_pp) {
        ...
        $query_p3at = mysqli_query($conn, "SELECT noref_pp FROM p3at WHERE noref_pp = '$noref'");
...
function DeleteKKSO($data) {
    ...
    $directory = 'files/opname/';
    // Membuka direktori
    if ($handle = opendir($directory)) {
        // Membaca file satu per satu
        while (false !== ($file = readdir($handle))) {
            // Memeriksa apakah file diawali dengan prefix yang ditentukan
            if (strpos($file, $no_so) === 0) {
                // Menghapus file
        ...
        $GLOBALS['alert'] = array("Gagal!", "Gagal mengakses direktori file opname noref ".$no_so, "error", "$page");
        return false;
    }
...
function ResetLHSO($data) {
    ...
    while($data_detail = mysqli_fetch_assoc($query_detail)){
        ...
        mysqli_query($conn, "UPDATE asset_stock_opname SET kondisi_so_asset = NULL, lokasi_so_asset = NULL, date_so_asset = NULL, photo_so_asset = NULL, user_so_asset = NULL, edtdate_so_asset = NULL, edited_so_asset = NULL WHERE pluid_so_asset = '$pluid'");
    ...
    if ($handle = opendir($directory)) {
        ...
        $GLOBALS['alert'] = array("Gagal!", "Gagal membuka direktori file opname noref ".$no_so, "error", "$page");
        return false;
    }
...
function UploadSO($data) {
    ...
    $user = mysqli_real_escape_string($conn, strtoupper($_POST['user-so']));
    $date = date("Y-m-d H:i:s");
    ...
    if (is_uploaded_file($tmp)) {
        ...
        while (($line = fgetcsv($file)) !== FALSE) {
            ...
            // Insert data to database
            $sql_asset = "UPDATE asset_stock_opname SET kondisi_so_asset = '$stbarang', lokasi_so_asset = '$lkbarang', user_so_asset = '$user', edtdate_so_asset = '$date', edited_so_asset = 'Y' WHERE noref_so_asset = '$noref' AND pluid_so_asset = '$kdbarang' AND sn_so_asset = '$snbarang' AND noat_so_asset = '$atbarang'";
...
function InsertPeriodePenilaianTahunan($data) {
    ...
    $sql_leader = "SELECT A.lvl_lead_user, A.nik_lead_user, A.div_lead_user, B.nik_head_lead_user, LEFT(B.name_sublead_user, 10) AS nik_lead FROM leader_users AS A
    ...
    if(mysqli_num_rows($query_leader) > 0 ) {
        while($data_leader = mysqli_fetch_assoc($query_leader)){
            ...
            $dl = $data_leader["div_lead_user"];
            $lvl = $data_leader["lvl_lead_user"];
            mysqli_query($conn, "INSERT INTO leader_assessment (officer_leader_assest, junior_leader_assest, div_leader_assest, lvl_leader_assest, head_code_sts_assest) VALUES ('$l', '$j', '$dl', '$lvl', '$idmax')");
        }
    }
    else {
    ...
    foreach ($div as $arr)  {
        ...
        $query_headiv = mysqli_query($conn, "SELECT id_head_divisi FROM divisi WHERE id_divisi = '$i'");
        $data_headiv = mysqli_fetch_assoc($query_headiv);

        $hd = isset($data_headiv["id_head_divisi"]) ? $data_headiv["id_head_divisi"] : NULL;

        mysqli_query($conn, "INSERT INTO divisi_assessment (head_code_sts_assest, head_id_divisi, head_idhead_divisi) VALUES ('$idmax', '$i', '$hd')");
    }
...
function UpdatePeriodePenilaianTahunan($data) {
    ...
    $sql_leader = "SELECT A.nik_lead_user, A.lvl_lead_user, A.div_lead_user, B.nik_head_lead_user, LEFT(B.name_sublead_user, 10) AS nik_lead FROM leader_users AS A
    INNER JOIN subleader_users AS B ON A.nik_lead_user = B.nik_head_lead_user
    ...
    if(mysqli_num_rows($query_leader) > 0 ) {
        ...
        while($data_leader = mysqli_fetch_assoc($query_leader)){
            ...
            $dl = $data_leader["div_lead_user"];
            $lvl = $data_leader["lvl_lead_user"];
            mysqli_query($conn, "INSERT INTO leader_assessment (lvl_leader_assest, div_leader_assest, officer_leader_assest, junior_leader_assest, head_code_sts_assest) VALUES ('$lvl', '$dl', '$l', '$j', '$code')");
        }
    }
    else {
        $GLOBALS['alert'] = array("Gagal!", "Tidak ada data leader yang terdaftar, masing-masing user harap mengupdate data leader terlebih dahulu", "error", "$page");
...
function CreatePenilaianTahunan($data) {
    ...
    foreach ($dataToInsert as $value) {
        ...
        if ($poin_indikator >= 90) {
            $mutuFix = 'A';
        } else if ($poin_indikator >= 81) {
            $mutuFix = 'B+';
    ...
    mysqli_query($conn, "INSERT INTO data_assessment (head_id_sts_assest, head_code_sts_assest, docno_data_assest, date_data_assest, office_data_assest, dept_data_assest, div_data_assest, lvl_data_assest, th_data_assest, leader_data_assest, junior_data_assest, poin_data_assest, mutu_data_assest, avg_data_assest, note_data_assest) VALUES ('$id_tahun', '$doc', '$idmax', '$date', '$office', '$dept', '$div', '$lvl', '$tahun', '$senior', '$junior', '$nilai', '$mutu', '$avg', '$note')");
...
function LaporPenilaianTahunanLeader($data) {
    ...
    $page = htmlspecialchars(mysqli_real_escape_string($conn, $data["page-laporbuatperiodeassest"]));
    $id = htmlspecialchars(mysqli_real_escape_string($conn, $data["id-laporbuatperiodeassest"]));
    $code = htmlspecialchars(mysqli_real_escape_string($conn, $data["code-laporbuatperiodeassest"]));
    $tahun = htmlspecialchars(mysqli_real_escape_string($conn, $data["tahun-laporbuatperiodeassest"]));
    $senior = htmlspecialchars(mysqli_real_escape_string($conn, $data["leader-laporbuatperiodeassest"]));
    $name = htmlspecialchars(mysqli_real_escape_string($conn, strtoupper($data["name-laporbuatperiodeassest"])));
    $office = htmlspecialchars(mysqli_real_escape_string($conn, $data["office-laporbuatperiodeassest"]));
    $divisi = htmlspecialchars(mysqli_real_escape_string($conn, $data["divisi-laporbuatperiodeassest"]));
    $transk = htmlspecialchars($data["trans-laporbuatperiodeassest"]);
    $date = date("Y-m-d H:i:s");

    $query_draft = mysqli_query($conn, "SELECT * FROM leader_assessment WHERE officer_leader_assest = '$senior' AND head_code_sts_assest = '$code' AND status_leader_assest = 'N'");

    if (mysqli_num_rows($query_draft) > 0) {

        $GLOBALS['alert'] = array("Gagal!", "Terdapat data karyawan yang belum dilakukan draft penilaian", "error", "$page");
        return false;
